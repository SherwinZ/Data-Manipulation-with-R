...
You can check the data type by `class()` function, as the code on the right shows.
...
## 2.3 Arithmetic based on vectors
...
**Selecting consecutively multiple elements**
...
**Selecting by names and calculating aggregate arithmetic**
...
selection_vector <- (poker_vector > 0L)
...
## 3.4 Aggregate row and col calculating in matrix
...
# Calculate series box office figures
...
- to paste together `star_wars_matrix` and `star_wars_matrix2`, in this order. Assign the resulting matrix to `all_wars_matrix`.
...
```
 ...
 [`cbind()`](http://www.rdocumentation.org/packages/base/functions/cbind) has [`rbind()`](http://www.rdocumentation.org/packages/base/functions/cbind), [`colSums()`](http://www.rdocumentation.org/packages/base/functions/colSums) has [`rowSums()`](http://www.rdocumentation.org/packages/base/functions/colSums). 
...
- `my_matrix[,1]` selects all elements of the first column. Don't put a : in the first arg `my_matrix[:,1]` <- this is not an R syntax
...
## 3.7 Arithmetic with elements in matrices
...
*Those who are familiar with matrices should note that this is not the standard matrix multiplication defined in **Linear Algebra**, for which you should use %\*% in R.*
